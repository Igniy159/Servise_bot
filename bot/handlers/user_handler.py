...
from core.schemas import (QueryReceiveUser,
                          CmdCreateUser,
                          CmdDeleteUser,
                          CmdChangeUser,
                          CmdRenameUser)
from bot.handlers.branch_handler import get_branches
from bot.keyboards.branch_keyboard import get_branch_keyboard
from bot.keyboards.main_keyboard import get_button_main
from bot.keyboards.user_keyboard import (get_user_for_owner_menu,
                                         get_roles_keyboard,
...
user_router = Router()
@user_router.callback_query(F.data == 'users')
async def users_menu(callback: CallbackQuery):
    menu = get_user_for_owner_menu()
    await callback.message.edit_text(text='Панель управления сотрудниками', reply_markup=menu)
...
async def start_create_user(callback: CallbackQuery,
    ...
    await callback.answer()
    await callback.message.edit_text('Введите TG id сотрудника:')
...
async def processing_user_role(callback: CallbackQuery,
    ...
    if role == Role.OWNER:
        ...
        await callback.message.edit_text(text="Выберите филиал: ", reply_markup=keyboard)
...
async def get_users_menu(callback: CallbackQuery,
    ...
    if data_type == 'branch_users':
        ...
        name = ctx.uow.branches.get({'branch_id': obj_id})[0].name
    else:
        command = QueryReceiveUser(depart_id=obj_id)
        name = ctx.uow.dep_mapper.get_depart_name(obj_id)
        name = ctx.formatter.translate(name)
    users = ctx.service_user.get(ctx.actor,command)
    if not users:
        menu = get_button_main()
        await callback.message.answer("Пользователей нет ", reply_markup=menu)
    else:
        keyboard = get_user_keyboard(users, ctx.formatter)
        await callback.message.edit_text(
            text=f"Сотрудники {name}: ",
            reply_markup=keyboard)

@user_router.callback_query(UserState.user_operations, F.data.startswith('user_id'))
async def get_user_operations(callback: CallbackQuery,
    ...
    users = ctx.uow.users.get({'user_id': obj_id,
                              'user_activity':1})
    user_name = users[0].name if users else ""
    await state.update_data(user_id=obj_id,
                            user_name=user_name)
    await callback.message.edit_text(text=f'Выберите действие над {user_name}:',
                                  reply_markup=keyboard)
...
async def operations_from_user(callback:CallbackQuery,
    ...
    if action == 'rename':
        ...
        await callback.message.edit_text(f'Введите новое имя для {user_name}: ')
    elif action == 'delete':
        data = await state.get_data()
        command = CmdDeleteUser(user_id=data['user_id'])
        user = ctx.service_user.delete(actor=ctx.actor, cmd=command)
        await callback.message.answer(
            f'Пользователь: {ctx.formatter.user_formatter(user)} был удалён',reply_markup=menu)
        await state.clear()
    elif action == 'change':
        await state.set_state(UserState.waiting_for_role)
        await state.update_data(mode='change')
        keyboard = get_roles_keyboard(ctx.formatter, 'change_user')
        await callback.message.edit_text(f'Выберите новую роль для {user_name}: ', reply_markup=keyboard)
...
async def rename_user(message: Message,
    ...
    await message.answer(
        f'Пользователь: {ctx.formatter.user_formatter(user)} был изменён',reply_markup=menu)
    await state.clear()
...
@user_router.callback_query(F.data == 'my_users')
async def manager_menu(callback: CallbackQuery):
    keyboard = get_user_for_manager_menu()
    await callback.message.edit_text(text='Панель управления пользователями: ',
                                  reply_markup=keyboard)
...
async def create_my_user_finish(message: Message,
    ...
    menu = get_button_main()
    await message.answer(
        f'Пользователь: {ctx.formatter.user_formatter(user)} был добавлен', reply_markup=menu)
    await state.clear()
...
async def delete_my_user(callback: CallbackQuery,
    ...
    users = ctx.service_user.get(ctx.actor, command)
    keyboard = get_user_keyboard(users, ctx.formatter)
    await callback.message.edit_text('Выберите сотрудника для удаления', reply_markup=keyboard)
...
async def delete_my_user_finish(callback: CallbackQuery,
    ...
    menu = get_button_main()
    await callback.message.answer(
        f'Пользователь: {ctx.formatter.user_formatter(deletable)} был удалён', reply_markup=menu)
    await state.clear()
...
async def rename_my_user_(callback: CallbackQuery,
    ...
    await callback.message.edit_text('Введите новое имя для сотрудника:')
...
async def rename_my_user_finish(message: Message,
    ...
    menu = get_button_main()
    await message.answer(
        f'Пользователь: {ctx.formatter.user_formatter(user)} был переименован',reply_markup=menu)
    await state.clear()
