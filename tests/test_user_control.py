...
def test_assert_first_user(db,config):
    create_first_owner('Владыка Тьмы',666,config,con=db)
    user = get_users_with_data(con=db)[0]
...
def test_owner_assert_users(db,config):
    ...
    gend_user.create_user("Фродо",1111,1,branch_id=1)  #employee
    gend_user.create_user("Бильбо", 1245, 2, branch_id=1)  #manager
    gend_user.create_user("Пипин",1456,3, depart_id=7)  #specialist
    gend_user.create_user("Мерлин", 7777,4)   #2 owner
